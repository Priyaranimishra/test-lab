...
ECR_APPLICATION_REPO_NAME=application-tier
...
ECR_PRESENTATION_REPO_NAME=presentation-tier
...
docker build -t application-tier .
docker tag application-tier:latest $ECR_APPLICATION_TIER_REPO:latest
...
docker tag presentation-tier:latest $ECR_PRESENTATION_TIER_REPO:latest
